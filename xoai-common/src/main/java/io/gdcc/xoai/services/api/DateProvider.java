...
public interface DateProvider {
    Map<Granularity, DateTimeFormatter> formatMap =
            Map.of(
    ...
    /**
     * Format an {@link Instant} as UTC instant ({@link Granularity#Second}). Example:
     * "2022-05-19T10:00:00Z"
     *
     * @param date The instant to format. By definition, an {@link Instant} is not zoned and always
     *     in UTC!
     * @throws DateTimeException when a necessary clock unit is not available from the timestamp
     * @deprecated As OAI-PMH is pretty explicit about using the same Granularity everywhere, you
    ...
    /**
     * Format an {@link Instant} with a given {@link Granularity}. Example: Granularity "Second":
     * "2022-05-19T10:00:00Z", Granularity "Day": "2022-05-19"
     *
     * @param date The instant to format. By definition, an {@link Instant} is not zoned and always
     *     in UTC!
    ...
    static Instant parse(String date, Granularity granularity) {
        if (granularity == Granularity.Day)
            return LocalDate.parse(date, formatMap.get(Granularity.Day))
                    .atStartOfDay()
                    .toInstant(ZoneOffset.UTC);
        else
            // first try to parse with second granularity
            try {
                return LocalDateTime.parse(date, formatMap.get(Granularity.Second))
                        .toInstant(ZoneOffset.UTC);
            } catch (DateTimeException e) {
                // if that failed, and we configured to be lenient, try again with day precision
                if (granularity == Granularity.Lenient) return parse(date, Granularity.Day);
