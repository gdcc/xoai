...
public class ParametersToUrlTest {
    ...
    public void fromUntilDatesPassed() {
        ...
        Parameters parameters =
                new Parameters().withVerb(Type.ListRecords).withFrom(date).withUntil(date);

        String url = parameters.toUrl(BASE_URL);
        assertEquals(
                BASE_URL
                        + "?verb=ListRecords"
                        + "&from=2011-04-07T09%3A30%3A00Z"
                        + "&until=2011-04-07T09%3A30%3A00Z",
                url);
    }
    ...
    public void differentGranularityIsRespected() {
        ...
        String url = parameters.toUrl(BASE_URL);
        assertEquals(
                BASE_URL + "?verb=ListRecords" + "&from=2011-04-07" + "&until=2011-04-07", url);
    }
}
