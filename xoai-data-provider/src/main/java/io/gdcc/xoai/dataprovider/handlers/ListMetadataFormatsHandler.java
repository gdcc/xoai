...
public class ListMetadataFormatsHandler extends VerbHandler<ListMetadataFormats> {
    ...
    public ListMetadataFormats handle(Request request) throws HandlerException {
        ...
        // This verb allows to specify an identifier to return the available metadata formats for this particular item
        Optional<String> identifier = request.getIdentifier();
        if (identifier.isPresent()) {
            ItemIdentifier item = itemRepository.getItem(identifier.get());
            
            // Lookup the formats available from the context. When the metadata formats registered within the context
            // do not define any condition, this will be a simple copy of all formats. When items might have not every
            // format available, it is up to the implementing application to provide a suiting condition to lookup
