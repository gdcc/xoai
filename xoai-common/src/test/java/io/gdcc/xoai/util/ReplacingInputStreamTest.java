...
/**
 * Source copied from
 * https://github.com/mwiesiolek/inbot-utils/blob/master/src/test/java/io/inbot/utils/ReplacingInputStreamTest.java
 * Originally licensed under MIT license Slightly adapted to make use of JUnit5 and Hamcrest instead
 * of TestNG etc
 *
...
public class ReplacingInputStreamTest {
    ...
    public static Stream<Arguments> samples() {
        return Stream.of(
                ...
                Arguments.of(
                        ...
                        "def",
                        "fed",
                        "abcfedghijkabcfedghijkabcfedghijk"),
    ...
    public void shouldReplace(String input, String pattern, String replacement, String expected)
            throws IOException {
        ByteArrayInputStream bis = new ByteArrayInputStream(input.getBytes(StandardCharsets.UTF_8));
        ...
        try (ReplacingInputStream ris =
                new ReplacingInputStream(
                        bis, pattern.getBytes(StandardCharsets.UTF_8), replacementBytes)) {
            String result = new String(ris.readAllBytes(), StandardCharsets.UTF_8);
            assertThat(result, equalTo(expected));
        }
    }
    ...
    @ParameterizedTest
    @MethodSource("samples")
    public void shouldReplaceUsingStringConstructor(
            String input, String pattern, String replacement, String expected) throws IOException {
        ByteArrayInputStream bis = new ByteArrayInputStream(input.getBytes(StandardCharsets.UTF_8));
